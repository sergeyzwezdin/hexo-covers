...
const getCoversToGenerate = (config, base_dir, source_dir, coverDir, manifest, globalData, log) => {
    // Find all posts
    ...
    coversToGenerate = [
        ...coversToGenerate,
        ...generatePageFileDescriptors(base_dir, source_dir, pages, config, globalData, templates.page, 'pages', log)
    ];
    ...
    coversToGenerate = [
        ...coversToGenerate,
        ...generatePageFileDescriptors(base_dir, source_dir, posts, config, globalData, templates.post, 'posts', log)
    ];
...
const invalidateCovers = (hexo) =>
    async function () {
        ...
        const globalData = {
            blogTitle: config.title || hexo.config.title
        };
        ...
        const coversToGenerate = getCoversToGenerate(config, base_dir, source_dir, coverDir, manifest, globalData, log);
        ...
        const processed = [...coversToGenerate.map(({ fileName }) => fileName), ...cleanupResult];
        ...
        if (!isCheckOnlyMode) {
            if (processed.length > 0) {
                ...
                log.info('Covers loaded in %s', cyan(prettyHrtime(process.hrtime(timeStarted))));
            }
        } else {
            ...
            if (processed.length === 0) {
                ...
                log.warn('The following items should be updated:\n%s', magenta(processed.map((f) => `      ${f}`).join('\n')));
...
module.exports = invalidateCovers;
